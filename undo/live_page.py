"""
ui/pages/live_page.py
Trang Session — realtime recording, transcript EN/VI, summary.
Pipeline: AudioCapture → VADProcessor → TranscriptionWorker → UI/DB

THAY ĐỔI so với version cũ:
    + _draft_timer (QTimer 1s): gọi self._vad.emit_draft() mỗi giây trong lúc ghi.
    + _on_draft_chunk(): nhận draft PCM từ VAD → enqueue(is_final=False).
    + Tất cả chỗ khác giữ nguyên.

Kết quả: text xuất hiện sau ~1s người bắt đầu nói (draft),
         rồi được confirm/replace khi VAD emit final chunk.
"""
